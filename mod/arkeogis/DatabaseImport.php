		...
		foreach($lines as $datas) {
			...
			if (!isset(self::$_stored[self::$_current[0]])) {
				...
				if (self::_checkIntegrity())
					self::$_stored[self::$_current[0]] = self::$_current;
				else {
					self::_addError(self::$_current[0], $lineNumber, "Wrong definition for site");
				}
			} else {
				// --
				// Link partial infos to previous stored infos
				// --
			}
		}
