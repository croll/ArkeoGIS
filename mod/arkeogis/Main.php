...
class Main {
  ...
  public static function hook_mod_arkeogis_export_sheet($hookname, $userdata) {
    if (!\mod\user\Main::userIsLoggedIn())
		...
		$columns="si_code, si_name, si_description, si_city_id, ST_AsGeoJSON(si_geom) as coords, si_centroid, si_occupation, si_creation, si_modification"; // ark_site
    ...
    $columns.=", sp_knowledge_type, sp_comment, sp_bibliography"; // ark_site_period
    ...
    fputcsv($fp, array('SITE_ID_SOURCE', 'BASE_SOURCE', 'NOM_SITE', 'NOM_COMMUNE_PRINCIPALE', 'CODE_COMMUNE', 'SYSTEME_PROJECTION', 'LONGITUDE_X ', 'LATITUDE_Y', 'LONGITUDE_X_BIS', 'LATITUDE_Y_BIS', 'ALTITUDE Z', 'CENTRE_COMMUNE', 'ETAT_CONNAISSANCES', 'OCCUPATION', 'DATATION_DEBUT_PLUS_FINE', 'DATATION_FIN_PLUS_FINE', 'IMMO_NIV1', 'IMMO_NIV2', 'IMMO_NIV3', 'IMMO_NIV4', 'PROFONDEUR_VESTIGES', 'IMMO_EXP', 'MOB_NIV1', 'MOB_NIV2', 'MOB_NIV3', 'MOB_NIV4', 'MOB_EXP', 'PROD_NIV1', 'PROD_NIV2', 'PROD_NIV3', 'PROD_EXP', 'BIBLIOGRAPHIE', 'REMARQUES'), ";", '"');
      ...
      fputcsv($fp, array(
                ...
                $row['da_name'],                 // BASE_SOURCE
                $row['si_name'],                 // NOM_SITE
                $row['ci_name'],                 // NOM_COMMUNE_PRINCIPALE
                $row['ci_code'],                 // CODE_COMMUNE
                'WGS84',                         // SYSTEME_PROJECTION
                $coords->coordinates[0],         // LONGITUDE_X
                $coords->coordinates[1],         // LATITUDE_Y
                '',                              // LONGITUDE_X_BIS
                '',                              // LATITUDE_Y_BIS
                $coords->coordinates[2] == -999 ? '' : $coords->coordinates[2] ,       // ALTITUDE Z
                $row['si_centroid'],             // CENTRE_COMMUNE
                $strings['knowledge'][$row['sp_knowledge_type']],  // ETAT_CONNAISSANCES
                $strings['occupation'][$row['si_occupation']],     // OCCUPATION
                count($period_start) ? $period_start[count($period_start)-1] : '', // DATATION_DEBUT_PLUS_FINE
                count($period_end) ? $period_end[count($period_end)-1] : '',       // DATATION_FIN_PLUS_FINE
                ...
                $row['sp_exceptional'],                            // PROD_EXP
                $row['sp_bibliography'],                           // BIBLIOGRAPHIE
                $row['sp_comment']                                 // REMARQUES
              ), ";", '"');
		}
